				...
				if socks == client:
					...
					if message: #MESSAGE WAS RECEIVED
						...
						#IF CHAIN HAS BEEN FULLY DOWNLOADED NETWORK REQUESTS A TARGET FOR THE NEXT BLOCK
							...
							random_number = random.randint(lower_bound,upper_bound) + (1048576*len(self.txn_pool))#4096#16777216)#,286331153)#572662306)#1431655765)#268435456,#858993459) #TARGET IS A 8 BYTE INTEGER
	...
	def add_txn_to_pool(self,txn):
		...
		if txn not in self.txn_pool: #ENSURE TXN IS NOT ALREADY IN TXN POOL
			...
			if self.node_target != None:
				...
				self.node_target = int(int.from_bytes(bytes.fromhex(self.node_target),byteorder='big') + 1048576).to_bytes(4,byteorder='big').hex()
