...
def parse_client_recvd(node,outgoing_client,msg):
	...
	if 'type' in msg_object and msg_object['type'] == 2:
		#print("Received Type 2 {0}".format(time.time()))
...
def parse_server_recvd(node, incoming_client, msg):
	...
	if 'type' in msg_object and msg_object['type'] == 0:
		...
		try:
			...
			if node.controller.chain_downloaded:
				message = {"type":12}
				node.client_broadcast(json.dumps(message))
		except:
			return
	...
	if 'type' in msg_object and msg_object['type'] == 2:
		#print("Received Type 2 {0}".format(time.time()))
		#node.controller.index_chain(save=False)
	...
	if 'type' in msg_object and msg_object['type'] == 9:
		if msg_object["forward"]:
			msg_object["forward"] = False
			node.client_broadcast(json.dumps(msg_object))
	...
	if 'type' in msg_object and msg_object['type'] == 12:
		if incoming_client not in node.controller.confirmation_nodes:
			node.controller.confirmation_nodes.append(incoming_client)
